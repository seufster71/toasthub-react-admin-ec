...
import ECMarketAdminModifyView from '../../adminViewec/market/market-modify-view';
import ECMarketAdminView from '../../adminViewec/market/market-view';
import BaseContainer from '../../core/container/base-container';

function ECMarketAdminContainer({location,navigate}) {
	const itemState = useSelector((state) => state.ecadminmarket);
	...
	const onSave = () => {
		BaseContainer.onSave({state:itemState,actions:actions,dispatch:dispatch,appPrefs:appPrefs,form:"EC_MARKET_ADMIN_FORM"});
	}
	const closeModal = () => {
	...
	const onOption = (code,item) => {
		fuLogger.log({level:'TRACE',loc:'MarketAdminContainer::onOption',msg:" code "+code});
		if (BaseContainer.onOptionBase({state:itemState,actions:actions,dispatch:dispatch,code:code,appPrefs:appPrefs,item:item})) {
			return;
		}
	}
	
	fuLogger.log({level:'TRACE',loc:'MarketAdminContainer::render',msg:"Hi there"});
	if (itemState.view == "MODIFY") {
		return (
			<ECMarketAdminModifyView
			itemState={itemState}
			appPrefs={appPrefs}
			onSave={onSave}
			onCancel={onCancel}
...
export default ECMarketAdminContainer;
