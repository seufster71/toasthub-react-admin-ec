...
import StoreModifyView from '../../adminViewec/store/store-modify-view';
import StoreView from '../../adminViewec/store/store-view';
import BaseContainer from '../../core/container/base-container';

function StoreContainer({location,navigate}) {
	const itemState = useSelector((state) => state.ecadminstore);
	...
	const onSave = () => {
		BaseContainer.onSave({state:itemState,actions:actions,dispatch:dispatch,appPrefs:appPrefs,form:"ADMIN_STORE_FORM"});
	}
	const closeModal = () => {
	...
	const onOption = (code,item) => {
		fuLogger.log({level:'TRACE',loc:'StoreContainer::onOption',msg:" code "+code});
		if (BaseContainer.onOptionBase({state:itemState,actions:actions,dispatch:dispatch,code:code,appPrefs:appPrefs,item:item})) {
			return;
		}
	}
	
	fuLogger.log({level:'TRACE',loc:'StoreContainer::render',msg:"Hi there"});
