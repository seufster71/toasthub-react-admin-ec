...
export function init({parent,parentType}) {
	return function(dispatch) {
		...
		requestParams.service = "ADMIN_STORE_SVC";
		requestParams.prefTextKeys = new Array("ADMIN_STORE_PAGE");
		requestParams.prefLabelKeys = new Array("ADMIN_STORE_PAGE");
		let params = {};
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "ADMIN_STORE_INIT", responseJson });
			} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		let prefChange = {"page":"STORE","orderCriteria":requestParams.orderCriteria,"listStart":requestParams.listStart,"listLimit":requestParams.listLimit};
		dispatch({type:"STORE_PREF_CHANGE", prefChange});
		let params = {};
		params.requestParams = requestParams;
		params.URI = '/api/admin/callService';
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "ADMIN_STORE_LIST", responseJson, paginationSegment });
				if (info != null) {
...
export function listLimit({state,listLimit}) {
	return function(dispatch) {
		 dispatch({ type:"ADMIN_STORE_LISTLIMIT",listLimit});
...
export function search({state,searchCriteria}) {
	return function(dispatch) {
		 dispatch({ type:"ADMIN_STORE_SEARCH",searchCriteria});
...
export function modifyItem({id, appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "ADMIN_STORE_SVC";
	    requestParams.prefFormKeys = new Array("ADMIN_STORE_FORM");
	    if (id != null) {
	    	requestParams.itemId = id;
	    }
	    let params = {};
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'ADMIN_STORE_ITEM',responseJson});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function inputChange(field,value) {
	 return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"ADMIN_STORE_INPUT_CHANGE",params});
	 };
}

export function searchChange({value}) {
	 return function(dispatch) {
		 dispatch({ type:"ADMIN_STORE_SEARCH_CHANGE",value});
	 };
}

export function orderBy({state,orderCriteria}) {
	 return function(dispatch) {
		 dispatch({ type:"ADMIN_STORE_ORDERBY",orderCriteria});
...
export function clearCategory() {
	return function(dispatch) {
		dispatch({ type:"ADMIN_STORE_CLEAR_CATEGORY"});
	};
}
...
export function clearField(field) {
	return function(dispatch) {
		let params = {};
		 params.field = field;
		dispatch({ type:"ADMIN_STORE_CLEAR_FIELD",params});
	};
}

export function setStatus({successes,errors}) {
	 return function(dispatch) {
		 dispatch({ type:"ADMIN_STORE_SET_STATUS",successes,errors});
	 };
}

export function openDeleteModal({item}) {
	 return function(dispatch) {
		 dispatch({type:"ADMIN_STORE_OPEN_DELETE_MODAL",item});
	 };
}

export function closeDeleteModal() {
	 return function(dispatch) {
		 dispatch({type:"ADMIN_STORE_CLOSE_DELETE_MODAL"});
	 };
}

export function cancel({state}) {
	return function(dispatch) {
		dispatch({type:"ADMIN_STORE_CANCEL"});
		dispatch(list({state}));
	 };
}